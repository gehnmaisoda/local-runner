---
name: lr
description: LocalRunner のタスク管理を行う。タスクの一覧表示・作成・編集・削除・実行・停止・ログ確認・設定変更などを lr CLI 経由で実行する。ユーザーが「タスクを作りたい」「ジョブを確認したい」「スケジュールを変更したい」「実行履歴を見たい」「設定を変えたい」などと言ったときに使う。
user-invocable: true
